from flaskbp.utils.UserDict import UserDict
...
class EnvironmentConfig(UserDict):
    ...
    def data(self):
        if self._data is None:
            self._data = {}
            self._data.update(dict_from_object('flaskbp.config.configs.' + self.environment.capitalize() + 'Config'))
            
            try:
                self._data.update(dict_from_object('flaskbp.config.config_local'))
            except ImportError:
                pass
